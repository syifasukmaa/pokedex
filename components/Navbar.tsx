...
const Navbar = () => {
  ...
  return (
    <nav className='bg-third fixed top-0 start-0 w-full shadow-md border-gray-200 dark:bg-gray-900 z-50'>
      <div
        className={
          open
            ? 'max-w-screen-xl z-50 flex flex-wrap items-center justify-center md:justify-between mx-auto p-4'
            : ' max-w-screen-xl z-50 flex flex-wrap items-center md:justify-between mx-auto p-4 justify-between md:py-4 md:px-16'
        }
      >
        <a
        ...
        >
          <ul className='font-medium flex flex-col items-center p-4 md:p-0 mt-4 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700'>
            {navbarItems.map((items, index) => (
              <li
                key={index}
                className='block py-2 px-3 text-black text-dark md:bg-transparent md:p-0 dark:text-white md:dark:text-blue-500 underline'
              >
                <button onClick={() => handleClick(items.id)}>{items.label}</button>
              </li>
            ))}
            <li className='block py-2 px-3 text-black rounded md:bg-transparent md:p-0 dark:text-white md:dark:text-blue-500'>
